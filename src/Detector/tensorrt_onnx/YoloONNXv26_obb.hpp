...
///
/// \brief The YOLOv26_obb_onnx class
///
class YOLOv26_obb_onnx : public YoloONNX
{
public:
	YOLOv26_obb_onnx(std::vector<std::string>& inputTensorNames, std::vector<std::string>& outputTensorNames)
	{
	...
	std::vector<tensor_rt::Result> GetResult(size_t imgIdx, int /*keep_topk*/, const std::vector<float*>& outputs, cv::Size frameSize)
	{
		...
		for (size_t i = 0; i < len; ++i)
		{
			...
			if (classConf >= m_params.m_confThreshold)
			{
				...
				float width = fw * output[ind + 2];
				float height = fh * output[ind + 3];
				float angle = 180.f * output[ind + 6] / static_cast<float>(M_PI);
				//std::cout << "ind = " << ind << ", output[0] = " << output[ind + 0] << ", output[1] = " << output[ind + 1] << ", output[2] = " << output[ind + 2] << ", output[3] = " << output[ind + 3] << std::endl;
				//std::cout << "ind = " << ind << ", classConf = " << classConf << ", classId = " << classId << ", x = " << x << ", y = " << y << ", width = " << width << ", height = " << height << ", angle = " << angle << std::endl;

				resBoxes.emplace_back(classId, classConf, cv::RotatedRect(cv::Point2f(x, y), cv::Size2f(width, height), angle));
			}
		}
