...
public:
    ...
    void Solve(const distMatrix_t& costMatrix, size_t colsTracks, size_t rowsRegions, assignments_t& assignmentT2R, track_t /*maxCost*/) override
    {
        //std::cout << "SPHungrian::Solve" << std::endl;
        m_solver.Solve(costMatrix, colsTracks, rowsRegions, assignmentT2R, AssignmentProblemSolver::optimal);
    }
...
///
/// \brief The SPLAPJV class
///
class SPLAPJV final : public ShortPathCalculator
{
public:
    SPLAPJV(const SPSettings& settings)
        : ShortPathCalculator(settings)
    {
        //std::cout << "SPLAPJV" << std::endl;
    }
