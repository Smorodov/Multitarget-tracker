    ...
    node_map<std::list<direction_indicator> >& dirs)
{
    ...
    std::vector<bool> turn(st_[*it], false);
    ...
    pq_tree* PQ)
{
    ...
    if (fail->kind() == pq_node::Q_NODE) {
        ...
        node tmp_node;
        forall_nodes(tmp_node, G) {
            upward_begin[tmp_node] = em.adjacency(tmp_node).begin();
        }
    ...
    q_node* q_fail)
{
    ...
    for (pq_tree::sons_iterator it = q_fail->sons.begin(), end = q_fail->sons.end(); it != end; ++it) {
        mark_all_neighbors_of_leaves(*it, below);
    }
