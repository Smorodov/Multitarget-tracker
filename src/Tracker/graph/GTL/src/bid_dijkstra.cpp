...
int bid_dijkstra::check(GTL::graph& G)
{
	...
	for (auto node_it = G.nodes_begin(); node_it != nodes_end; ++node_it)
	{
		if (*node_it == s)
		{
			source_found = true;
	...
	for (auto edge_it = G.edges_begin(); edge_it != edges_end; ++edge_it)
	{
...
int bid_dijkstra::run(GTL::graph& G)
{
	init(G);

	double max_dist = 1;
	for (auto edge_it = G.edges_begin(); edge_it != G.edges_end(); ++edge_it)
	{
	...
	bin_heap<node, less_dist> source_heap(source_prd, G.number_of_nodes());
	bin_heap<node, less_dist> target_heap(target_prd, G.number_of_nodes());
	...
	while ((!source_heap.is_empty()) || (!target_heap.is_empty()))
	{
			...
			target_dist[target_heap.top()])
		{
			...
			for (auto adj_edge_it = cur_node.adj_edges_begin();
				...
				++adj_edge_it)
			{
				...
				if (source_mark[op_node] == black)
				{
					...
					source_dist[op_node] = source_dist[cur_node] + weight[*adj_edge_it];
					...
					{
						if (max_dist > source_dist[op_node] + target_dist[op_node])
							max_dist = source_dist[op_node] + target_dist[op_node];
					}
				}
				else if (source_mark[op_node] == grey)
				{
					if (source_dist[op_node] > source_dist[cur_node] + weight[*adj_edge_it])
					{
						source_dist[op_node] = source_dist[cur_node] + weight[*adj_edge_it];
				...
				{
					// nothing to do: shortest distance to op_node is already computed
				}
			}
		}
			...
			//  target_dist[target_heap.top()])
		{
			...
			if (G.is_directed())
			{
				...
				for (auto in_edge_it = cur_node.in_edges_begin();
					...
					++in_edge_it)
				{
					...
					if (target_mark[op_node] == black)
					{
						...
						target_dist[op_node] = target_dist[cur_node] + weight[*in_edge_it];
					...
					{
						if (target_dist[op_node] > target_dist[cur_node] + weight[*in_edge_it])
						{
							target_dist[op_node] = target_dist[cur_node] + weight[*in_edge_it];
...
void bid_dijkstra::init(GTL::graph& G)
{
	...
	if (path_set)
	{
		pred.init(G, GTL::edge());
		succ.init(G, GTL::edge());
