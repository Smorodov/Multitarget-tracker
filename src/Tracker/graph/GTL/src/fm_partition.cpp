...
int fm_partition::check(GTL::graph& G)
{
...
int fm_partition::run(GTL::graph& G)
{
	init_variables(G);
...
void fm_partition::hide_self_loops(GTL::graph& G)
{
...
void fm_partition::shuffle_vector(const int vector_size, std::vector<graph::node_iterator>& node_vector)
{
...
void fm_partition::copy_side_node_map(const graph& G, node_map<side_type>& dest, const node_map<side_type> source) const
{
	graph::node_iterator node_it = G.nodes_begin();
	graph::node_iterator nodes_end = G.nodes_end();
	while (node_it != nodes_end)
...
bool fm_partition::move_vertex(const graph& G, GTL::node& moved_node)
{
	node cons_nodeA;
	if (!bucketA_empty)
...
void fm_partition::update_bucketB(const node cur_node, const int old_gain, const int new_gain)
{
	if (fixed[cur_node] != UNFIXED)
		return;	// fixed nodes need no update
