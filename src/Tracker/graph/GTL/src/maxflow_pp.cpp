...
void maxflow_pp::set_vars(const edge_map<double>& edge_capacity, const node& net_source, const node& net_target)
{
	this->edge_capacity = edge_capacity;
...
int maxflow_pp::check(GTL::graph& G)
{
...
int maxflow_pp::run(GTL::graph& G)
{
	// init
	if (artif_source_target)
...
void maxflow_pp::create_artif_source_target(GTL::graph& G)
{
...
int maxflow_pp::leveling(GTL::graph& G)
{
...
void maxflow_pp::hide_unreachable_nodes(GTL::graph& G)
{
...
void maxflow_pp::min_throughput_node(const graph& G, GTL::node& min_tp_node,
	double& flow_value)
{
...
void maxflow_pp::get_sp_ahead(const graph& G, const node& start_node, node_map<edge>& last_edge)
{
	std::queue<node> next_nodes;
...
void maxflow_pp::get_sp_backwards(const graph& G, const node& start_node, node_map<edge>& prev_edge)
{
	std::queue<node> next_nodes;
	node_map<bool> visited(G, false);
	next_nodes.push(start_node);
...
void maxflow_pp::push(GTL::graph& G, const node& start_node, const double flow_value)
{
...
void maxflow_pp::pull(GTL::graph& G, const node& start_node, const double flow_value)
{
...
void maxflow_pp::comp_rem_net(GTL::graph& G)
{
	// update back_edges
	graph::edge_iterator edge_it = G.edges_begin();
	graph::edge_iterator edges_end = G.edges_end();
...
void maxflow_pp::single_edge_update(GTL::graph& G, GTL::edge cur_edge)
{
...
double maxflow_pp::extra_charge_ahead(const node& start_node, const node_map<edge>& last_edge) const
{
...
double maxflow_pp::extra_charge_backwards(const node& start_node, const node_map<edge>& prev_edge) const
{
	...
	double min_value = edge_capacity[prev_edge[cur_node]] - edge_max_flow[prev_edge[cur_node]];

	do
	{
		double cur_capacity = edge_capacity[prev_edge[cur_node]]
			- edge_max_flow[prev_edge[cur_node]];
		if (cur_capacity < min_value) min_value = cur_capacity;
		cur_node = prev_edge[cur_node].target();
	} while (cur_node != start_node);
...
void maxflow_pp::create_back_edge(GTL::graph& G, const edge& org_edge)
{
...
void maxflow_pp::restore_graph(GTL::graph& G)
{
	G.restore_graph();
